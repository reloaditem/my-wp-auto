...
def get_blog_content():
    ...
    try:
        ...
        for line in lines:
            ...
            if len(clean_line) < 65 and (clean_line[0].isdigit() or clean_line.endswith(':') or clean_line.istitle()):
                ...
                content_parts.append(f'<p style="line-height:2.0; margin-bottom:20px; font-size:1.1em;">{clean_line}</p>')

        content_body = "".join(content_parts)
        for i in range(len(final_images)):
            img_tag = f'<figure style="text-align:center; margin:30px 0;"><img src="{final_images[i]}" style="width:100%; border-radius:15px; box-shadow:0 10px 20px rgba(0,0,0,0.1);"></figure>'
