#!/usr/bin/env python3
"""
recover_autopost_style_since_2026-02-12.py

Scan WordPress posts (publish + future) since AFTER_DATE_KST (default 2026-02-12 KST).
If content is missing/broken, regenerate a full autopost-like article:
- Raw HTML output (no ``` fences); fences are stripped if present
- Pricing mentions stripped
- Tables wrapped for mobile scroll
- Body images ensured via Unsplash (BODY_IMAGE_COUNT)
- Internal links + related posts block
- Print-friendly checklist section (clean, not the broken print stub)
- Original HTML backed up to BACKUP_DIR before overwrite
- Featured image (thumbnail) stays as-is (we only update "content")

ENV:
  WP_BASE, WP_USER, WP_PASS, OPENAI_API_KEY (required)
  UNSPLASH_ACCESS_KEY (recommended for images)
...
def main():
    ...
    print(f"[INFO] DRY_RUN={DRY_RUN} MAX_FIX={MAX_FIX} MIN_WORDS={MIN_WORDS} INTERNAL_LINKS={INTERNAL_LINKS} BODY_IMAGE_COUNT={BODY_IMAGE_COUNT} MODEL={MODEL}")
