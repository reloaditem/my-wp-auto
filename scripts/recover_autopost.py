...
def ensure_body_images_with_target(html_text: str, topic: str, target_count: int) -> str:
    if target_count <= 0:
        return html_text
    soup = BeautifulSoup(html_text or "", "html.parser")
    ...
    urls = unsplash_search(topic, count=max(need, target_count))
    if not urls:
        return str(soup)
    ...
    insert_points = []
    if h2s:
        # Spread insert points across the article
        insert_points = [h2s[0]]
        if len(h2s) >= 2:
...
""".strip()
    ...
    html_out = ensure_body_images_with_target(html_out, title, target_images)
...
def main():
    ...
    for p in candidates:
        ...
        words = _plain_words(content_html)
        imgc = _body_image_count(content_html)
        target_imgs = get_image_target_by_category(cat_name)
        print(f"[FLAG] id={pid} words={words} imgs={imgc} target_imgs={target_imgs} cat={cat_name} flags={flags} title={title_plain!r}")
